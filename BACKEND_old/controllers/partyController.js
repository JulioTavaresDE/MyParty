...
const partyController = {
    create: async (req,res) => {
        try {
            ...
            if(party.services && !checkPartyBudget(party.budget,party.services)) {
                res.status(406).json({msg:"O seu orcamento e insuficiente"});
                return;
            }
    ...
    get: async(req,res) => {
        try {
            ...
            if(!party){
                res.status(404).json({msg:"Festa nao encointrada"})
                return;
            }
    ...
    delete:async (req,res) => {
        try {
            ...
            if(!party){
                party.status(404).json({msg:"Festa nao encontrada"})
                return
            }

            const deleteParty = await PartyModel.findByIdAndDelete(id)
            res.status(200).json({deleteParty,msg:"Festa excluidacom sucesso"})
    ...
    update:async (req,res) => {
        try {
            ...
            if(!updateParty){
                res.status(404).json({msg:"Festa nao encntrada"})
                return
            }

            res.status(200).json({party,msg:"Festa alerada com sucesso"})
